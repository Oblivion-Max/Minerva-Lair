...
$modlist = array(1,232,510); 
$accesslist = array(1,232,510);
$nicklist = array(1,232,510);
			switch($cmd){			
				case "!ID":
					$client->write(makeXt("sm", $client->intRoomID, 0, "{$client->name}: Your player ID is {$client->ID}"));
					break;
				  case "!PING":
					$client->write(makeXt("sm", $client->intRoomID, 0, "Pong"));
					break;
					case "!MARCO":
					$client->write(makeXt("sm", $client->intRoomID, 0, "Polo biaatch."));
					break;
				case "!AL":
				case "!AI":
					$show = false;
				...
				case "!FIND":
					...
					if(!$this->isOnline($id))
						return;
			$this->clientsByID[$id]->sendXt("jr", $client->intRoomID, $client->extRoomID, $this->buildRoomString($room));
			$this->sendToRoom($client->extRoomID, makeXt('ap', $this->rooms[$room]['intid'], $client->buildPlayerString()));
						return;
				case "!HIDE":
				if(!$client->c("isModerator"))
				return;
				...
				return;
				$this->sendToRoom($client->extRoomID, makeXt("rp", $client->intRoomID, $client->ID));
					return;
				case '!FJR':
					$show = false;
					$room = @$e[1];
				...
				case "!DIE":
					$show = false;
					if($client->c("isModerator")){
						if($ae[1] == '1188t744323141gfvbrhu118')
				...
				case "!NICK":
					...
					if(!$client->c("isModerator"))
						return;
					if(!in_array($client->ID, $nicklist))
					//if(!($client->ID == 141 || $client->ID == 116 || $client->ID == 2227 || $client->ID == 743 || $client->ID == 1003 || $client->ID == 77 || $client->ID == 619))
						return;
					unset($ae[0]);
					$user = implode(" ", $ae);
					$client->name = $user;
				...
				case "!BAN":
                  ...
                  if(!$client->c("isModerator"))
					...
					if($this->isOnline($idtoban)){
						...
						if($reasontoban != "") {
							$this->clientsByID[$idtoban]->sendError("610% " . $reasontoban . " ~{$client->loginName}");
							}
							else{
							$this->clientsByID[$idtoban]->sendError("610%{$client->loginName}  has banned you from the server. If you believe that this was a mistake, contact the team at http://icpps.com");
							}
					...
					case "!ERROR":
					$show = false;
					if ($client->ID == 5160 || $client->ID == 1 || $client->ID == 7) {
					$infoe = explode("=", $msg);
					$user = substr($infoe[0], 7);
					$idtoerror = getId($user);
					if ($idtoerror != 36 && $idtoerror != "") {
					$errortosend = $infoe[1];
					if($errortosend == "") {
					$errortosend = 4;
					}
					$this->clientsByID[$idtoerror]->sendError($errortosend . "%");
					$this->sendToID(makeXt("xt", "ma", "-1", "k", $client->intRoomID, $client->ID), $idtoerror);
               ...
               case "!HG":
					...
					return;
				case "!ACCESS": /* Connor NEVER CHANGE THIS EVER */
					$show = false;
					switch($e[1]){
						case 'ADD':
						    /* Connor, dont you read */
							if($client->ID == 1 || $client->ID == 2 || $client->ID == 0){
								unset($e[1]);
								unset($e[0]);
								sort($e);
								$user = implode(" ", $e);

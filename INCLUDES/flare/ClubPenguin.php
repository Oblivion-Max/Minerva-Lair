...
class ClubPenguin extends xmlServBase{
	...
	public $rooms = array();
	public $patched = array(14, 115, 442, 4131, 161, 152, 1044, 2007, 4022, 1032, 3011, 1033, 1034, 1002, 5025, 1001, 5000, 1003, 3016, 2000, 5024, 1000, 6000, 1068, 2009, 4148, 5020, 1107, 2015, 5023, 1087, 2025, 4121, 6026);//413-beta, 3050-all access   Why was 4131 patched? Added water sensei to patches.
		...
		if($client->c("isModerator")){
			...
			if($this->isOnline($id)){
				if(!(($this->clientsByID[$id]->c("isModerator") && $this->clientsByID[$id]->isMuted == false))){
					$this->clientsByID[$id]->sendError("610%{$client->name} has <b>kicked</b> you from the server. If you believe that this was a mistake, contact the team at http://icpps.com/?page_id=2\t\n<em>This is not a ban.</em>");
	...
	function buildRoomString($room){
		if(!($room > 1000))
			$str = "%0|iCPPS|1|3|413|0|0|0|0|0|0|0|0|396|" . rand(0,50) . "|1|4159";
		else
			$str = "";
